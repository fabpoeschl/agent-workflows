...
- **Run the failing test(s) locally first** to get the actual error message before reading any code.
- For CI-only failures (pass locally, fail on CI):
  - **Do not investigate without the full CI failure output including the seed.**
  - Ask the user for the CI output if not provided.
  - Reproduce with the exact seed locally.
...
- Clear the context after the task is complete.
...
- **Never investigate CI-only ordering-dependent failures without the full CI output and seed.**
